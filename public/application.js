...
var turn = '';
var isOver = false;
var dataPatch = function dataPatch(data) {
  // event.preventDefault()
  // const data = event.target.attr('data-space')
  // const turn = event.target.innerText
  var isOver = winCondition();
  api.updateGame(data, turn, isOver).then(ui.onPatchSuccess).catch();
};
var newGame = function newGame(event) {
  ...
  api.createGame().then(ui.onCreateGame);
...
var getGames = function getGames(event) {
  ...
  api.index().then(ui.onGetGamesSuccess);
};
...
var updateGame = function updateGame(data, turn, isOver) {
  return $.ajax({
